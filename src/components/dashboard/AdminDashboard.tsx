...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
    ...
    // State for Draft Modal (Hoisted)
    const [draftModal, setDraftModal] = useState<{ isOpen: boolean, ticketId: string | null, content: string }>({
        isOpen: false,
        ticketId: null,
        content: ''
    });
    ...
    const handleSendDraft = async () => {
        if (!draftModal.ticketId) return;

        try {
            // Dynamically import sendReply
            const { sendReply } = await import('../../apiService');

            await sendReply(draftModal.ticketId, draftModal.content);

            notifications.push({ id: Date.now(), msg: `Reply sent & emailed for ticket #${draftModal.ticketId}` });
            setDraftModal({ isOpen: false, ticketId: null, content: '' });
                                                                ...
                                                                <div className="flex gap-2">
                                                                    ...
                                                                    <button
                                                                        onClick={async (e) => {
                                                                            ...
                                                                            const draft = await draftReplyFromAI({
                                                                                id: ticket.id,
                                                                                title: ticket.title,
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col max-h-[90vh]">
                        <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-950">
                            <div>
                                <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                    <span className="text-xl">✨</span> AI Agent Copilot
                                </h3>
                                <p className="text-sm text-slate-500">Review and edit the generated response before sending.</p>
                            </div>
                            <button
                        ...
                        <div className="p-6 flex-1 overflow-y-auto">
                            <div className="mb-4 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-800 dark:text-indigo-300 px-4 py-3 rounded-xl text-sm border border-indigo-100 dark:border-indigo-900/50 flex items-start gap-3">
                                <div className="mt-0.5"><Shield className="w-4 h-4" /></div>
                                <p>This response was drafted by Llama 3 based on the ticket details and history. Please verify technical accuracy.</p>
                            </div>

                            <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">Draft Response</label>
                            <textarea
                                value={draftModal.content}
                                onChange={(e) => setDraftModal({ ...draftModal, content: e.target.value })}
                                className="w-full h-64 bg-slate-50 dark:bg-slate-800 border-none rounded-xl p-4 text-slate-700 dark:text-slate-200 focus:ring-2 focus:ring-indigo-500 outline-none resize-none font-sans text-sm leading-relaxed"
                            />
                        </div>
                        ...
                        <div className="p-6 border-t border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex justify-end gap-3">
                            <button
                                onClick={() => setDraftModal({ ...draftModal, isOpen: false })}
                                className="px-5 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-800 rounded-xl transition-colors"
                            >
                                Discard
                            </button>
                            <button
                                onClick={handleSendDraft}
                                className="px-5 py-2.5 text-sm font-bold text-white bg-indigo-600 hover:bg-indigo-700 rounded-xl shadow-lg shadow-indigo-600/20 transition-all active:scale-95 flex items-center gap-2"
                            >
                                <Check className="w-4 h-4" />
                                Send Reply
                            </button>
